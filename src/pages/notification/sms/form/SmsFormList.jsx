...
export default function SmsFormList() {
  ...
  const fetchSmsTemplateList = async (nextCursor = null, reset = false) => {
    ...
    try {
      ...
      const nextTotalCount = data?.totalCount ?? 0;
  ...
  const handleSearch = () => {
    setCursor(null);
    setHasNext(true);
    fetchSmsTemplateList(null, true);
  };

  const handleLoadMore = () => {
    if (!cursor) return;
    fetchSmsTemplateList(cursor, false);
  };
